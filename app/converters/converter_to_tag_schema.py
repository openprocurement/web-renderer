...
class HTMLToJSONTagSchemeConverter(BSToJSONConverter):
	...
	def create_for_loop_body(self, json, current_tag):
		current_text = current_tag.contents[0]
		tree = {'name': current_text}
		new_tag = self.soup.new_tag("form", id=current_text)
		if "new_tag" in json:
			json["new_tag"].append(new_tag)
			json["tree"][current_text] = tree

		json[str(current_tag)] = {
			"name": current_text,
			"tag": current_tag,
			"new_tag": new_tag,
			"tree": tree,
	...
	def create_for_loop_structure(self, current_tag, end_tag, tags, json=None):
		...
		while current_tag != end_tag:
			...
			if isinstance(current_tag, Element.Tag):
				...
				if (re.match(RegexConstants.FOR_LOOP_END_TAG, current_text)):
					return current_tag
				elif (re.match(RegexConstants.FOR_LOOP_BEGIN_TAG, current_text)):
					json = self.create_for_loop_body(json, current_tag)
					current_tag = self.create_for_loop_structure(
						current_tag, end_tag, tags, json[str(current_tag)])
				else:
					cur_tag = deepcopy(current_tag)
					json["tree"][current_text] = current_text
					json["new_tag"].append(cur_tag)
		return
		

	def create_for_loop_inheritance(self, soup):
		"""
	...
	def convert(self, formatted_html):
		...
		self.soup = self.create_for_loop_inheritance(self.soup)
		self.remove_empty_tags(self.soup)
		self.soup = self.replace_tags_to_new(
			self.soup, HTMLToJSONTagSchemeConverter.TO_INPUT_FIELDS, HTMLToJSONTagSchemeConverter.INPUT_FIELD)
