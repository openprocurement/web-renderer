...
def json_decode_error_handler(error):
    return format_exception(error, "json_data is not in valid JSON format ", 500)


@app.errorhandler(docx.opc.exceptions.PackageNotFoundError)
def docx_package_not_found_error(error):
    return format_exception(error, "Template file is invalid.", 500)


@app.errorhandler(jinja2.exceptions.TemplateError)
def jinja2_undefined_error(error):
    return format_exception(error, "Template values do not match data_json: %s" % str(*error.args), 500)
...
def method_not_allowed_handler(error):
    return format_exception(error, "Method is not allowed.", 405)


@app.errorhandler(werkzeug.exceptions.BadRequestKeyError)
def bad_request_key_error(error):
    return format_exception(error, error.description, 500)


@app.errorhandler(re.error)
def regex_error(error):
    return format_exception(error, str(*error.args), 500)
...
def custom_exceptions_error_handler(error):
    if isinstance(error, CustomException):
        return format_exception(error, error.description, error.code)
...
def file_not_found_handler(error):
    return format_exception(error, str(*error.args), 404)
